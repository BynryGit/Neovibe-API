...
def validation_assignment():
    try:
        validation_assignment_obj = ValidationAssignment.objects.filter(is_active=True)
        for validation in validation_assignment_obj:
            meter_reading_obj = MeterReading.objects.filter(reading_status=0, is_assign_to_v1=False, is_active=True,
            ...
            meter_reading_obj = MeterReading.objects.filter(reading_status=1, is_assign_to_v2=False, is_active=True,
                                                            is_duplicate=False, read_cycle_id=validation.read_cycle_id)
            for meter_reading in meter_reading_obj:
