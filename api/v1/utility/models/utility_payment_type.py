...
class UtilityPaymentType(models.Model):
...
def get_utility_payment_type_by_id(id):
    try:
        return UtilityPaymentType.objects.filter(id=id)
    except Exception as e:
        raise CustomAPIException("Payment Type does not exists.", status_code=status.HTTP_404_NOT_FOUND)


def get_utility_payment_type_by_id_string(id_string):
    try:
        return UtilityPaymentType.objects.get(id_string=id_string)
    except:
        return False
