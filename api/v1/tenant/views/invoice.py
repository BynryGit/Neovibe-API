...
from v1.tenant.models.tenant_invoices import TenantInvoices as tenantInvoicesTbl
...
# API Header
# API end Point: api/v1/tenant/invoice/list
# API verb: GET
# Package: Basic
# Modules: All
# Sub Module: All
...
class TenantInvoiceList(generics.ListAPIView):
    serializer_class = TenantInvoiceListSerializer
    ...
    def get_queryset(self):
        queryset = tenantInvoicesTbl.objects.filter(is_active=True)
        return queryset


#
# # API Header
# # API end Point: api/v1/tenant/invoice
# # API verb: POST
# # Package: Basic
# # Modules: Tenant
# # Sub Module: Invoice
# # Interaction: Add Tenant Subscription Plan
...
#                            tenant_obj = serializer.create(serializer.validated_data, user)
                            tenant_invoice_obj = serializer.create(serializer.validated_data)
                            view_serializer = TenantInvoiceViewSerializer(instance=tenant_invoice_obj, context={'request': request})
                            return Response({
                                STATE: SUCCESS,
...
# Modules: All
# Sub Module:
# Interaction: Get Tenant, Update Tenant Invoice
# Usage: Add and Update Tenant Invoices in the system
# Tables used:  Tenant Invoices
# Auther: Gauri Deshmukh
# Created on: 21/5/2020
...
class TenantInvoiceDetail(GenericAPIView):
    ...
    def get(self, request, id_string):
        try:
            if is_token_valid(1):
                if is_authorized():
                    tenant_invoice = get_tenant_invoice_by_id_string(id_string)
                    if tenant_invoice:
                        serializer = TenantInvoiceViewSerializer(instance=tenant_invoice, context={'request': request})
                        return Response({
                            STATE: SUCCESS,
                    ...
                    if is_data_verified(request):
                        ...
                        if tenant_invoice_obj:
                            ...
                            if serializer.is_valid(request.data):
                                ...
                                tenant_invoice_obj = serializer.update(tenant_invoice_obj, serializer.validated_data)

                                view_serializer = TenantInvoiceViewSerializer(instance=tenant_invoice_obj,
