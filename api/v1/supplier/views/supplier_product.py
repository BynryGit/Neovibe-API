    ...
    try:
        ...
        def get_queryset(self):
            if is_token_valid(self.request.headers['token']):
                if is_authorized():
                    supplier_product_obj = get_supplier_product_by_id_string(self.kwargs['id_string'])
                    if supplier_product_obj:
                        queryset = SupplierProductTbl.objects.filter(supplier=supplier_product_obj.id, is_active=True)
                        return queryset
                    else:
        ...
        try:
            ...
            if is_token_valid(request.headers['token']):
                ...
                # Checking authorization end
                    ...
                    duplicate_supplier_product_obj = SupplierProductTbl.objects.filter(utility__id_string=request.data['utility'], name=request.data['name'])
                    if duplicate_supplier_product_obj:
                        return Response({
                            STATE: DUPLICATE,
                        ...
                        if serializer.is_valid():
                            supplier_product_obj = serializer.create(serializer.validated_data, user)
                            return Response({
                                STATE: SUCCESS,
                                RESULTS: {'supplier_product_id_string': supplier_product_obj.id_string},
