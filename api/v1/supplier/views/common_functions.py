...
from v1.supplier.models.supplier_type import get_supplier_type_by_id_string
from v1.supplier.models.product_category import get_supplier_product_category_by_id_string
from api.messages import SUPPLIER_TYPE_NOT_FOUND, SUPPLIER_SUBTYPE_NOT_FOUND, PRODUCT_CATEGORY_NOT_FOUND,PRODUCT_SUBCATEGORY_NOT_FOUND,UTILITY_NOT_FOUND,TENANT_NOT_FOUND 
...
def set_supplier_type_vaidated_data(validated_data):
...
def set_supplier_subtype_vaidated_data(validated_data):
    if "utility_id" in validated_data:
        utility = get_utility_by_id_string(validated_data["utility_id"])
        if utility:
            validated_data["utility_id"] = utility.id
    ...
    if "tenant_id" in validated_data:
        ...
        if tenant:
            ...
            raise CustomAPIException(TENANT_NOT_FOUND, status_code=status.HTTP_404_NOT_FOUND)
    if "supplier_type_id" in validated_data:
        supplier_type = get_supplier_type_by_id_string(validated_data["supplier_type_id"])
        if supplier_type:
            validated_data["supplier_type_id"] = supplier_type.id
        else:
            raise CustomAPIException(SUPPLIER_TYPE_NOT_FOUND, status_code=status.HTTP_404_NOT_FOUND)
    return validated_data

def set_product_category_vaidated_data(validated_data):
    if "utility_id" in validated_data:
        utility = get_utility_by_id_string(validated_data["utility_id"])
        if utility:
            validated_data["utility_id"] = utility.id
...
def set_product_subcategory_vaidated_data(validated_data):
    if "utility_id" in validated_data:
        utility = get_utility_by_id_string(validated_data["utility_id"])
        if utility:
            validated_data["utility_id"] = utility.id
    ...
    if "category" in validated_data:
        product_category = get_supplier_product_category_by_id_string(validated_data["category"])
        if product_category:
            validated_data["category"] = product_category.id
        else:
            raise CustomAPIException(PRODUCT_CATEGORY_NOT_FOUND, status_code=status.HTTP_404_NOT_FOUND)
    return validated_data
