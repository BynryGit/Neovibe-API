...
class CustomFilter:
    ...
    def get_filtered_queryset(queryset, request):
            ...
            if consumer_master_objs:
                for consumer_master_obj in consumer_master_objs:
                    ...
                    queryset = ConsumerServiceContractDetail.objects.filter(
                        consumer_id__in=[consumer.id for consumer in consumer_master_list], is_active=False, state=2)
        ...
        if 'consumer_processing_history' in request.query_params:
            ...
            if consumer_master_objs:
                for consumer_master_obj in consumer_master_objs:
                    ...
                    # consumer = get_consumer_by_id_string(self.kwargs['id_string'])
                    queryset = ConsumerServiceContractDetail.objects.filter(
                        consumer_id__in=[consumer.id for consumer in consumer_master_list], is_active=True, state=0,
                        created_date__gte=datetime.now() - timedelta(days=180))
                    # queryset = CustomFilter.get_filtered_queryset(queryset, self.request)

        if 'route_history' in request.query_params:
            route_list = []
            route_obj = ConsumerMaster.objects.filter(is_active=True, state=0)
            if route_obj:
                for routes in route_obj:
                    route_obj.append(routes)
                    queryset = ConsumerServiceContractDetail.objects.filter(
                        consumer_id__in=[consumer.id for consumer in route_list], is_active=True, state=0,
                        created_date__gte=datetime.now() - timedelta(days=180))
            ...
            if utility_work_order_type_obj:
                ...
                if work_order_master_obj:
                    ...
                    queryset = ServiceAppointment.objects.filter(
                        work_order_master_id__in=[i.id for i in work_order_master_obj], state=11, is_active=False)
                    print("++++++++++", queryset)
            ...
            if utility_work_order_type_obj:
                ...
                if work_order_master_obj:
                    ...
                    queryset = ServiceAppointment.objects.filter(
                        ...
                        created_date__gte=datetime.now() - timedelta(days=180), state__in=[7, 10])
                    print("++++++++++", queryset)
        ...
        if 'Registration_history' in request.query_params:
            queryset = RegTbl.objects.filter(is_active=True, state=1,
                                             created_date__gte=datetime.now() - timedelta(days=180))
        ...
        if 'Payment_history' in request.query_params:
            queryset = Payment.objects.filter(is_active=True, created_date__gte=datetime.now() - timedelta(days=180),
                                              state=1)
