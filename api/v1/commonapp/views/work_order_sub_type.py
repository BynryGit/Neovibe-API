...
from v1.commonapp.serializers.work_order_sub_type import WorkOrderSubTypeSerializer, WorkOrderSubTypeViewSerializer, WorkOrderSubTypeListSerializer
...
# Usage: API will fetch all Work Order SubType list
# Tables used: WorkOrderSubType
# Author: Chinmay
# Created on: 03/1/2021


class WorkOrderSubTypeList(generics.ListAPIView):
    try:
        serializer_class = WorkOrderSubTypeListSerializer
...
# API Header
# API end Point: api/v1/work-order-sub-type
# API verb: POST
# Package: Basic
# Modules: Admin
# Sub Module: Admin
# Interaction: WorkOrderSubType Post
# Usage: API will POST Work Order Sub Type into database
# Tables used: WorkOrderSubType
# Author: Chinmay
# Created on: 03/1/2021
class WorkOrderSubType(GenericAPIView):
    ...
    def post(self, request):
        try:
            ...
            serializer = WorkOrderSubTypeSerializer(data=request.data)
            if serializer.is_valid(raise_exception=False):
                work_order_sub_type_obj = serializer.create(serializer.validated_data, user)
                view_serializer = WorkOrderSubTypeViewSerializer(instance=work_order_sub_type_obj, context={'request': request})
                return Response({
                    STATE: SUCCESS,
...
class WorkOrderSubTypeDetail(GenericAPIView):
    ...
    def get(self, request, id_string):
        try:
            ...
            if work_order_sub_type:
                serializer = WorkOrderSubTypeViewSerializer(instance=work_order_sub_type, context={'request': request})
                return Response({
                    STATE: SUCCESS,
    ...
    def put(self, request, id_string):
        try:
            user_id_string = get_user_from_token(request.headers['Authorization'])
            user = get_user_by_id_string(user_id_string)
            work_order_sub_type_obj = get_work_order_sub_type_by_id_string(id_string)
            if "name" not in request.data:
                request.data['name'] = work_order_sub_type_obj.name
            if work_order_sub_type_obj:
                serializer = WorkOrderSubTypeSerializer(data=request.data)
                if serializer.is_valid(raise_exception=False):
                    work_order_sub_type_obj = serializer.update(work_order_sub_type_obj, serializer.validated_data, user)
                    view_serializer = WorkOrderSubTypeViewSerializer(instance=work_order_sub_type_obj,
