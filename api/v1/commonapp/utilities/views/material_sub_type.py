...
class MaterialSubTypeUpload(GenericAPIView):
    @is_token_validate
    ...
    def post(self, request, id_string):
        try:
            with transaction.atomic():
                ...
                material_sub_type_values = []
                loc = (r"/home/ubuntu/smart360/api/v1/commonapp/utilities/xls_files/Material-Sub-Type-Final.xls")
                ...
                for row in range(1, number_of_rows):
                    ...
                    material_sub_type_values.append(row_values)

                for value in material_sub_type_values:
                    if MaterialSubTypeTbl.objects.filter(tenant=tenant_obj,name=value[1],material_type_id=value[2],is_active=True).exists():
                        pass
                    else:
                        MaterialSubTypeTbl(
                            tenant=tenant_obj,
                            name=value[1],
                            material_type_id=value[2],
                            is_active = value[3]
                        ).save()
                return Response({
                    STATE: SUCCESS,
