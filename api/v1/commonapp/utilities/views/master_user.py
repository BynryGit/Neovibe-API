...
from master.models import MasterUser as MasterUserTbl
...
class MasterUserUpload(GenericAPIView):
    @is_token_validate
        ...
        try:
            with transaction.atomic():
                ...
                for row in range(1, number_of_rows):
                    ...
                    master_user_values.append(row_values)

                for value in master_user_values:
                    if MasterUserTbl.objects.filter(tenant=tenant_obj,email=value[1], is_active=True).exists():
                        pass
                    else:
                        MasterUserTbl(
                            tenant=tenant_obj,
