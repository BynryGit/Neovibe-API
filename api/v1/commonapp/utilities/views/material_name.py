...
class MaterialNameUpload(GenericAPIView):
    @is_token_validate
    ...
    def post(self, request, id_string):
        try:
            with transaction.atomic():
                ...
                loc = (r"/home/ubuntu/smart360/api/v1/commonapp/utilities/xls_files/Material-Name-Final.xls")
                ...
                for row in range(1, number_of_rows):
                    ...
                    material_name_values.append(row_values)

                for value in material_name_values:
                    if MaterialNameTbl.objects.filter(tenant=tenant_obj,name=value[1],material_subtype_id=value[2],is_active=True).exists():
                        pass
                    else:
                        MaterialNameTbl(
                            tenant=tenant_obj,
                            name=value[1],
                            material_subtype_id=value[2],
                            is_active = value[3]
                        ).save()
                return Response({
                    STATE: SUCCESS,
